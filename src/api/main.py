...
class EvaluationResponse(BaseModel):
    ...
    chao_xung_danh: Optional[int] = Field(
        None, ge=0, le=1, description="Điểm chào/xưng danh (0 hoặc 1)"
    )
    ky_nang_noi: Optional[int] = Field(
        None, ge=0, le=1, description="Điểm kỹ năng nói (0 hoặc 1)"
    )
    ky_nang_nghe: Optional[int] = Field(
        None, ge=0, le=1, description="Điểm kỹ năng nghe (0 hoặc 1)"
    )
    thai_do: Optional[int] = Field(
        None, ge=0, le=1, description="Điểm thái độ (0 hoặc 1)"
    )
    ...
    segments: Optional[List[Dict[str, Any]]] = Field(
        None, description="Dữ liệu các đoạn âm thanh"
    )
    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "muc_loi": "M1",
                "ly_do": "- Chào hỏi tốt\n- Nói hơi nhanh\n- Chưa lắng nghe tốt",
                "metadata": {"duration": 120.5, "turns": 15, "ratio_sales": 0.65},
            }
        }
...
async def process_evaluation_task(task_id: str, audio_bytes: bytes):
    ...
    try:
        ...
        if not evaluation_result or "error" in evaluation_result:
            error_msg = (
                evaluation_result.get("error", "Lỗi không xác định")
                if evaluation_result
                else "Không nhận được response"
            )
        ...
        # Tính tổng điểm
        tong_diem = 0.2 * (chao_xung_danh + ky_nang_noi) + 0.8 * (
            ky_nang_nghe + thai_do
        )
        ...
        try:
            ...
            logger.warning(
                f"[{task_id}] Không thể lưu file (nhưng vẫn có kết quả): {e}"
            )
...
async def get_task_result(task_id: str):
    ...
    **Status codes:**
    ...
    if task_id not in task_storage:
        ...
        raise HTTPException(
            status_code=404, detail=f"Không tìm thấy task_id: {task_id}"
        )
...
async def get_statistics():
    ...
    if completed_tasks:
        avg_score = sum(t.get("tong_diem", 0) for t in completed_tasks) / len(
            completed_tasks
        )
