...
class QAMainEvaluator:
    ...
    async def evaluate_communication(
        self, audio_bytes: bytes, task_id: int, trace=None
    ):
        """
        ...
        try:
            ...
            evaluation_result = await get_qa_evaluation(
                data_for_llm, trace=trace, parent_span=span
            )
            ...
            return {"detail_result": str(e), "score": -1, "status": -1, "segments": []}

    async def evaluate_sale_skills(self, audio_bytes: bytes, task_id: int, trace=None):
        """
    ...
    async def run_evaluate(self, audio_bytes: bytes, task_id: int):
        ...
        try:
            ...
            communication_result = await self.evaluate_communication(
                audio_bytes, task_id, trace=trace
            )
